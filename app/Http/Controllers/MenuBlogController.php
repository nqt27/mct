...
class MenuBlogController extends Controller
{
    ...
    public function index()
    {
        ...
        $menu = MenuBlog::whereNull('parent_id') // Chỉ lấy menu cha
            ->orderBy('position') // Sắp xếp theo vị trí
            ->get();
        return view('admin.menu-blog',  ['menu' => $menu]);
    }
    public function submenu($id)
    {
        $menu = MenuBlog::with('submenu')->where('id', $id)->first();
        // Lấy tất cả menu cùng với menu con (nếu có)
        $submenu = MenuBlog::with('submenu')
            ->where('parent_id', $id)
            ->orderBy('position')
            ->get();
        return view('admin.submenu-blog',  ['menu' => $menu, 'submenu' => $submenu]);
    }
    public function store(Request $request)
    {
        ...
        $maxPosition = MenuBlog::max('position');
        // Tạo sản phẩm mới
        $menu = new MenuBlog;
        ...
        try {
            ...
            return redirect()->route('menu-blog.index')->with('success', 'Menu đã được thêm thành công!');
        } catch (\Exception $e) {
            // Xử lý lỗi và chuyển hướng về trang danh sách với thông báo thất bại
            return redirect()->route('menu-blog.index')->with('error', 'Có lỗi xảy ra khi thêm menu.');
        }
    }
    public function destroy($id)
    {
        // Tìm sản phẩm bằng ID
        $menu = MenuBlog::where('id', $id)->first();
        // Xóa sản phẩm
        $menu->delete();

        // Redirect lại trang danh sách sản phẩm với thông báo thành công
        return redirect()->route('menu-blog.index')->with('success', 'Product deleted successfully.');
    }
    public function addSub(Request $request)
    {
        ...
        return redirect()->route('menu-blog.index')->with('success', 'Product updated successfully.');
    }
    ...
    private function updateOrderRecursive($items)
    {
        foreach ($items as $index => $item) {
            ...
            $menuItem = MenuBlog::find($item); // Chỉ lấy menu cha
            if ($menuItem) {
