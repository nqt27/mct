...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('menu_dichvu', function (Blueprint $table) {
            $table->id(); // Khóa chính
            $table->string('name'); // Tên của menu
            $table->unsignedBigInteger('parent_id')->nullable(); // Tham chiếu đến menu cha
            $table->string('slug')->nullable(); // Đường dẫn của menu (nếu có)
            ...
            $table->foreign('parent_id')->references('id')->on('menu_dichvu')->onDelete('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('menu_dichvu');
    }
};
