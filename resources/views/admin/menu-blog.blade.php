                    ...
                    <div id="column-1">
                        ...
                        <div class="card menu-item" id="menu-{{$m->id}}">
                            <div class="card-header card-header-divider">{{$m->name}}
                                <a href="/admin/submenu-blog/{{$m->id}}" class="btn btn-info waves-effect waves-light btn-submenu" data-id="{{$m->id}}" data-name="{{$m->name}}" data-url="{{$m->url}}" type="button" style="float: right; margin:5px"><i class="fas fa-list mr-1"></i><span>Danh mục con</span></a>
...
<script type="text/javascript">
    $(document).ready(function() {
        drag('/admin/updateOrder-blog');
        addMenu($('#btn-add-menu'), '/admin/addMenu-blog', 'POST', null);
        editMenu($('.edit-btn'), `/admin/menu-blog`);
        deleteMenu(`/admin/menu-blog`);
    });
</script>
</body>
