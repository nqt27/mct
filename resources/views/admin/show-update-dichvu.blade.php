                ...
                <div class="col-md-12">
                    <div class="card card-border-color card-border-color-primary">
                        <div class="card-body" style="flex-direction: row; justify-content: normal">
                            ...
                            <a href="{{route('dichvu.index')}}" class="btn btn-secondary waves-effect waves-light" type="button"><span>Trở về</span></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-7">
                    <div class="card card-border-color card-border-color-primary">
                        ...
                        <div class="card-body">
                            <form style="width: 100%;" id="audio-info-form" method="post" action="{{ route('dichvu.update', $dichvu->id) }}">
                                @csrf
                                <div class="form-group row">
                                    <label class="col-sm-3 control-label">Đường dẫn mẫu</label>
                                    <div class="col-sm-9">
                                        <p class="form-control-static" id="url-simple">{{$dichvu->slug}}</p>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-md-3 control-label"></label>
                                    <div class="col-sm-9">
                                        <input class="form-control" type="text" required="" placeholder="Đường dẫn" value="{{$dichvu->slug}}" name="slug" id="audio-url">
                                    </div>
                                </div>
                                <div class=" form-group row">
                                    <label class="col-md-3 control-label">Tiêu đề bài viết</label>
                                    <div class="col-sm-9">
                                        <input class="form-control" type="text" required="" placeholder="Tiêu đề bài viết" value="{{$dichvu->tieude}}" name="tieude" id="name">
                                    </div>
                                </div>
                                <div class="form-group row">
                    ...
                    <div class="card card-border-color card-border-color-primary">
                        ...
                        <div class="card-body">
                            <form style="width: 100%;" id="audio-seo-form" method="post" action="{{ route('dichvu.store') }}">
                                @csrf
                                <div class="form-group row">
                                    <label class="col-md-3 control-label">Keywword chính: </label>
                                    <div class="col-sm-9">
                                        <input class="form-control" type="text" placeholder="Keywword chính" name="keyword_focus" id="keyword_focus" value="{{$dichvu->keyword_focus}}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-md-3 control-label">SEO Title:</label>
                                    <div class="col-sm-9">
                                        <input class="form-control" type="text" placeholder="seo_title" name="seo_title" id="seo_title" value="{{$dichvu->seo_title}}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-md-3 control-label">SEO Keywords:</label>
                                    <div class="col-sm-9">
                                        <input class="form-control" type="text" placeholder="seo_keywords" name="seo_keywords" value="{{$dichvu->seo_keywords}}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-md-3 control-label">SEO Description:</label>
                                    <div class="col-sm-9">
                                        <textarea name="seo_description" style="width: 100%; height: 120px; padding: 10px;" id="seo_description" placeholder="SEO Description">{{$dichvu->seo_keywords}}</textarea>
                                    </div>
                                </div>
                ...
                <div class="col-md-5">
                            ...
                            <div class="form-group" style="justify-content: space-between; align-items: center;">
                                ...
                                <div style="padding: 0" class="col-12 col-sm-8 col-lg-6">
                                    <div class="switch-button switch-button-success switch-button-xs">
                                        <input type="checkbox" class="status-checkbox" data-switch="success" {{ $dichvu->display ? 'checked' : '' }} name="display" id="display{{$dichvu->id}}">
                                        <label for="display{{$dichvu->id}}" data-on-label="Yes" data-off-label="No"></label>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group" style="justify-content: space-between; align-items: center;">
                                ...
                                <div style="padding: 0" class="col-12 col-sm-8 col-lg-6">
                                    <div class="switch-button switch-button-success switch-button-xs">
                                        <input type="checkbox" class="status-checkbox" data-switch="success" {{ $dichvu->moi ? 'checked' : '' }} name="moi" id="is_new{{$dichvu->id}}">
                                        <label for="is_new{{$dichvu->id}}" data-on-label="Yes" data-off-label="No"></label>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card card-border-color card-border-color-primary">
                        ...
                        <div class="card-body">
                            ...
                            <form style="max-width: 100%" method="post" action="{{route('dichvu.store')}}" enctype="multipart/form-data" class="dropzone dz-clickable col-sm-9" id="my-dropzone">
                                @csrf
    ...
    $(document).ready(function() {
        ...
        var myDropzone = new Dropzone("#my-dropzone", {
            ...
            init: function() {
                ...
                var existingImage = "{{ asset('images/' . $dichvu->image) }}";
                if (existingImage) {
        ...
        $('#submit-all').on('click', function(e) {
            ...
            var dichvuId = "{{ $dichvu->id }}"; // Lấy ID sản phẩm từ PHP
            //Gửi request AJAX với FormData đã gộp
            $.ajax({
                url: `/admin/dichvu/${dichvuId}`, // Đường dẫn route
                method: 'POST',
                data: formData,
                ...
                success: function(response) {
                    ...
                    Swal.fire({
                        ...
                        window.location.href = '/admin/dichvu';
                    });
